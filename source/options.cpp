...
void Options::Init()
{
	...
	*/
	...
	m_pSoundOn->m_X = IwGxGetScreenWidth() * 0.65f + m_pSoundOn->m_W * 0.5f;
	m_pSoundOn->m_Y = m_pSound->m_Y;
	AddChild(m_pSoundOn);

	m_pSoundOff = g_pSpriteManager->CreateSpriteObject(Game::kGameGraphics_Options_Off);
	m_pSoundOff->m_X = IwGxGetScreenWidth() * 0.65f + m_pSoundOff->m_W * 0.5f;
	...
	*/
	...
	m_pTutorialOn->m_X = IwGxGetScreenWidth() * 0.65f + m_pTutorialOn->m_W * 0.5f;
	...
	m_pTutorialOff->m_X = IwGxGetScreenWidth() * 0.65f + m_pTutorialOff->m_W * 0.5f;
